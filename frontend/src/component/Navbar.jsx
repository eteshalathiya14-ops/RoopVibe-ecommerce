        ...
        <div style={{ display:'flex', alignItems:'center', height:52, padding:'0 14px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:20 }}>
            <button onClick={() => setIsSearchOpen(s => !s)}
              style={{ background:'none', border:'none', cursor:'pointer', color:CHARCOAL, display:'flex', flexDirection:'column', alignItems:'center', gap:2, padding:0 }}>
              <FiSearch size={20} />
            ...
            {/* Profile → /profile page directly, no dropdown */}
        ...
        {isSearchOpen && (
          <form onSubmit={handleSearch} style={{ display:'flex', alignItems:'center', backgroundColor:'#fff', borderTop:`1px solid ${BORDER}`, padding:'8px 12px', gap:8 }}>
            <input autoFocus type="text" placeholder="Search products, brands..."
              ...
              style={{ flex:1, border:`1.5px solid ${GOLD}`, outline:'none', borderRadius:8, padding:'8px 12px', fontSize:14, color:CHARCOAL, fontFamily:'inherit' }} />
      ...
      <div style={{ maxWidth:1400, margin:'0 auto', padding:'0 24px', height:68, display:'flex', alignItems:'center', position:'relative' }}>
          ...
          <nav style={{ display:'flex', alignItems:'center', height:68, flex:1 }}>
            {navLinks.map(link => (
              <div key={link.name} style={{ height:'100%', display:'flex', alignItems:'center' }}
                ...
                <Link to={link.special || link.name==='OFFERS' ? '/offers' : '#'}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:24, flexShrink:0 }}>
          ...
          <Link to="/wishlist" style={{ textDecoration:'none' }}>
            <button style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:2, background:'none', border:'none', cursor:'pointer', position:'relative' }}>
              <div style={{ position:'relative' }}>
                <FiHeart size={20} color={likedCount > 0 ? '#ff4d4f' : CHARCOAL} fill={likedCount > 0 ? '#ff4d4f' : 'none'} style={{ transition:'all 0.2s ease' }} />
                {likedCount > 0 && <span style={{ position:'absolute', top:-6, right:-7, backgroundColor:GOLD, color:'#fff', fontSize:9, fontWeight:700, width:16, height:16, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center' }}>{likedCount}</span>}
              </div>
              <span style={{ fontSize:9, fontWeight:700, letterSpacing:'0.8px', color: likedCount > 0 ? '#ff4d4f' : CHARCOAL }}>WISHLIST</span>
            </button>
          </Link>
          {/* Profile → /profile page directly, no dropdown on both web & mobile */}
