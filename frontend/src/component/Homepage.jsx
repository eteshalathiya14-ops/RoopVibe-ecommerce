/**
 * HomePage.jsx  — DYNAMIC VERSION
 * Banners, CategoryBar, Products all come from AdminDataContext.
 * Any changes in Admin will reflect here immediately.
 */
  ...
  return (
    <div style={{ backgroundColor:"#FAF7F2", borderBottom:`1px solid ${BORDER}` }}>
      ...
      <div style={{ display:"flex", alignItems:"flex-end", overflowX:"auto",
        ...
        {cats.map(cat => (
          <ArchCatItem key={cat.id} cat={cat} isActive={activeItem===cat.label}
            onClick={() => setActiveItem(cat.label)} isMobile={isMobile} />
        ))}
      </div>
    </div>
  );
    ...
    <div style={{ padding: isMobile ? "10px 0 8px":"14px 0 10px", backgroundColor:SURFACE, display:"flex", justifyContent:"center" }}>
      <div onMouseEnter={() => clearInterval(timerRef.current)} onMouseLeave={startTimer}
        ...
        <div style={{ position:"relative", height:H, borderRadius: isMobile ? 10:12, overflow:"hidden",
          ...
          background:slide.bg, transition:"background 0.6s" }}>
          ...
          {!isMobile && (
            <div style={{ position:"absolute", inset:0, pointerEvents:"none", overflow:"hidden" }}>
              <div style={{ position:"absolute", width:180, height:180, borderRadius:"50%",
                ...
                top:-40, left:-40, animation:"float1 5s ease-in-out infinite" }}/>
            </div>
          )}

          {/* Text */}
          <div key={`t${txtKey}`} style={{ position:"absolute", left:0, top:0, bottom:0, width:textW,
            padding:textPad, display:"flex", flexDirection:"column", justifyContent:"center", zIndex:2 }}>
            ...
            <h2 style={{ fontSize:titleSz, fontWeight:800, color:"#fff", lineHeight:1.15,
              ...
              whiteSpace:"pre-line", textShadow:"0 2px 14px rgba(0,0,0,0.35)", animation:"slideInL 0.6s 0.1s ease both" }}>
              {slide.title}
            </h2>
          ...
          <div style={{ position:"absolute", right:0, top:0, bottom:0, width:imgW, overflow:"hidden" }}>
            <img key={`i${imgKey}`} src={slide.img} alt=""
              style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition:"top",
          ...
          <div style={{ position:"absolute", bottom:10, right:10, backgroundColor:"rgba(26,20,8,0.55)",
            border:`1px solid ${GOLD}44`, padding:"2px 8px", borderRadius:20, fontSize:9,
            fontWeight:700, color:GOLD, zIndex:10 }}>
            {current+1}/{total}
          </div>

          {!isMobile && [[-1,"left"],[1,"right"]].map(([d,p]) => (
...
function ProductCard({ product, isMobile }) {
  ...
  // Use first color variant's first image as thumbnail
  const thumb    = product.colorVariants?.[0]?.images?.[0] || "";
  const thumb2   = product.colorVariants?.[0]?.images?.[1] || thumb;
  ...
  return (
    <div className="rv-prod-card" onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ backgroundColor:"#FAF7F2", borderRadius:8, overflow:"hidden",
        border:`1px solid ${hov ? GOLD:"#e8e8e8"}`, cursor:"pointer", display:"flex", flexDirection:"column" }}>
      <Link to={`/product/${product.id}`} style={{ textDecoration:"none", display:"flex", gap:4, padding:4 }}>
        <div style={{ flex:"1.5", height:mainH, borderRadius:4, overflow:"hidden" }}>
          <img src={thumb} alt={product.title} style={{ width:"100%", height:"100%", objectFit:"cover", display:"block" }}/>
        </div>
        <div style={{ flex:"0.6", display:"flex", flexDirection:"column", gap:4 }}>
          <div style={{ height:sideH, borderRadius:4, overflow:"hidden" }}>
            <img src={thumb2} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", filter:"blur(1px)", opacity:0.85 }}/>
          </div>
          <div style={{ position:"relative", height:sideH, borderRadius:4, overflow:"hidden" }}>
            ...
            <div style={{ position:"absolute", inset:0, backgroundColor:"rgba(0,0,0,0.4)",
              ...
              +{(product.colorVariants?.reduce((s,v)=>s+(v.images?.filter(Boolean).length||0),0)||0)}
            </div>
          </div>
        </div>
      </Link>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:4, marginTop:2, flexWrap:"wrap" }}>
            ...
            <span style={{ fontSize:9, fontWeight:700, color:"#2E7D32",
              backgroundColor:"#E8F5E9", padding:"1px 5px", borderRadius:3 }}>{discount}%</span>
          </div>
        </div>
          ...
          {!isMobile && (
            <button onClick={e=>{e.preventDefault();e.stopPropagation();addToCart(product);}}
              style={{ width:28, height:28, borderRadius:"50%", display:"flex", alignItems:"center",
                ...
                boxShadow:"0 2px 8px rgba(201,169,110,0.35)", border:"none", padding:0, cursor:"pointer" }}>
              <FiPlus size={18}/>
            </button>
          )}
        </div>
      ...
      <div style={{ maxWidth:1320, margin:"0 auto" }}>
        <div style={{ marginBottom: isMobile ? 18:28 }}>
          <h2 style={{ fontSize: isMobile ? "18px":"22px", fontWeight:700, color:CHARCOAL,
            fontFamily:"'Playfair Display',Georgia,serif", marginBottom:4 }}>Trending Right Now</h2>
          <p style={{ fontSize: isMobile ? "12px":"13px", color:MUTED, marginBottom:8 }}>Handpicked looks you'll love</p>
