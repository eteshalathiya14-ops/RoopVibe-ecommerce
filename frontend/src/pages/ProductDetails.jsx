/**
 * ProductDetailPage.jsx — FULLY FIXED v2
 * Fixes:
 * 1. Fullscreen image viewer — proper overlay with img display
 * 2. Related products — same navName/colTitle category only
 * 3. Related products — wishlist + add to cart buttons
 * 4. Reviews — persisted via localStorage per product
 */
...
function ImageViewer({ images, startIdx, onClose }) {
  ...
  return (
    ...
    >
      ...
      {/* Main image — stopPropagation so click on img doesn't close */}
      <div
        onClick={e => e.stopPropagation()}
        style={{
          maxWidth: "88vw", maxHeight: "88vh",
          display: "flex", alignItems: "center", justifyContent: "center",
          animation: "imgIn 0.25s ease",
        }}
      >
        <img
          key={idx}
      ...
      <button onClick={onClose}
        style={{
          ...
          background: "rgba(255,255,255,0.15)", border: "none", borderRadius: "50%",
          width: 42, height: 42, color: "#fff", cursor: "pointer",
          display: "flex", alignItems: "center", justifyContent: "center",
          zIndex: 100000,
        }}>
        <FiX size={20}/>
      ...
      {images.length > 1 && (
        <div
          ...
          style={{
            ...
            display: "flex", gap: 8, zIndex: 100000,
          }}
        >
          {images.map((img, i) => (
            <div key={i} onClick={() => setIdx(i)}
              style={{
                width: 48, height: 56, borderRadius: 6, overflow: "hidden", cursor: "pointer",
                border: `2.5px solid ${i === idx ? GOLD : "rgba(255,255,255,0.3)"}`,
...
function AddReviewForm({ onSubmit }) {
  ...
  const submit = () => {
    ...
    onSubmit({ name: name.trim(), rating, text: text.trim(), date, verified: false });
  ...
  return (
    <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 10, padding: 18, marginBottom: 24 }}>
      ...
      <button onClick={submit}
        style={{ padding: "10px 24px", background: GOLD, color: "#fff", border: "none", borderRadius: 6, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
        Submit Review
      </button>
    </div>
  );
...
// ── Related Product Card — with wishlist + cart ─────────────────
function RelatedCard({ p }) {
  ...
  return (
    <div className="pdp-related"
      style={{ backgroundColor: "#fff", borderRadius: 10, overflow: "hidden", border: `1px solid ${BORDER}`, display: "block", position: "relative" }}>
      <Link to={`/product/${rpid}`} style={{ textDecoration: "none", display: "block" }}>
        ...
        <div style={{ padding: "10px 14px 6px" }}>
          <p style={{ fontSize: 11, color: GOLD_DARK, fontWeight: 700, letterSpacing: "0.4px", marginBottom: 2, textTransform: "uppercase" }}>
            {p.brand || ""}
          </p>
      ...
      <div style={{ padding: "8px 14px 12px" }}>
        <button
          className="pdp-related-cart"
          onClick={handleCart}
          style={{
            width: "100%", padding: "9px", background: addedR ? GREEN : GOLD, color: "#fff", border: "none",
            borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer", transition: "background 0.2s",
...
export default function ProductDetailPage() {
  ...
  // ── Reviews: load from localStorage per product ─────────────
  const STORAGE_KEY = `rvibe_reviews_${pid}`;
  ...
  // Reload from storage when product id changes
  useEffect(() => {
    try {
      const saved = localStorage.getItem(`rvibe_reviews_${pid}`);
      if (saved) setReviewsList(JSON.parse(saved));
      else setReviewsList(null);
    } catch (_) {}
  }, [pid]);

  const reviews = reviewsList ?? (product?.reviews_list || []);

  const handleAddReview = (r) => {
    const updated = [r, ...reviews];
    setReviewsList(updated);
    try { localStorage.setItem(`rvibe_reviews_${pid}`, JSON.stringify(updated)); } catch (_) {}
  };
  ...
  const flatDetails = useMemo(() => {
    ...
    const LABELS = {
      ...
      fit:"Fit Type", washCare:"Wash Care", blouseIncluded:"Blouse", sareeLength:"Saree Length",
      setIncludes:"Set Includes", neckType:"Neck Type", sleeveType:"Sleeve Type",
      rise:"Rise", closure:"Closure", activity:"Activity / Sport", padding:"Padding",
      wiretype:"Wire Type", material:"Material", sole:"Sole Type", heelHeight:"Heel Height",
      plating:"Plating / Finish", stoneType:"Stone Type", threadCount:"Thread Count",
      color:"Color", dimensions:"Dimensions", capacity:"Capacity", safeFor:"Safe For",
    };
    const autoLabel = (k) => String(k).replace(/([a-z0-9])([A-Z])/g,"$1 $2").replace(/_/g," ").replace(/^./,c=>c.toUpperCase());
    const result = {};
    ["fabric","pattern","occasion","fit","washCare"].forEach(k => {
      if (product?.[k]) result[LABELS[k] || k] = product[k];
    });
    const d = product.details || {};
    Object.entries(d).forEach(([k, v]) => {
      if (k === "description") return;
      if (v === undefined || v === null) return;
      // Handle array values (from multi-select custom dropdown)
      if (Array.isArray(v)) {
        if (v.length === 0) return;
        const label = LABELS[k] || autoLabel(k);
        if (result[label] === undefined) result[label] = v.join(", ");
        return;
      }
  ...
  // ── Related products: same navName + colTitle, exclude current ──
  const relatedProds = useMemo(() => {
    if (!product) return [];
    return products.filter(p => {
      if (getPid(p) === String(id)) return false;
      if (!p.active) return false;
      // Same main category (navName) — strictly same colTitle if available
  ...
  return (
    <div style={{ backgroundColor: SURFACE, minHeight: "100vh" }}>
          ...
          {["Home", product.navName||"", product.colTitle||"", product.title].filter(Boolean).map((b, i, arr) => (
            <span key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span onClick={() => i === 0 && navigate("/")}
                style={{ cursor: i < arr.length - 1 ? "pointer" : "default", color: i === arr.length - 1 ? GOLD_DARK : MUTED, fontWeight: i === arr.length - 1 ? 600 : 400 }}>
                {b}
              </span>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 32, alignItems: "start" }}>
          ...
          <div style={{ position: "sticky", top: 80 }}>
            ...
            <div
              ...
              {images.length > 0 ? (
                <img key={`${varIdx}-${activeImg}`} src={images[activeImg] || images[0]} alt={product.title}
                  style={{ width: "100%", height: 420, objectFit: "cover", objectPosition: "top", display: "block", animation: "imgIn 0.3s ease" }}
                  onError={e => e.target.style.opacity = "0"}/>
              ) : (
              ...
              {discount > 0 && (
                <div style={{ position: "absolute", top: 12, left: 12, background: `linear-gradient(135deg,${GOLD_DARK},${GOLD})`, color: "#fff", fontSize: 11, fontWeight: 800, padding: "3px 9px", borderRadius: 4 }}>
                  {discount}% OFF
                </div>
              )}
              {/* Wishlist */}
              ...
              {images.length > 1 && (<>
                <button onClick={e => { e.stopPropagation(); setActiveImg(i => (i - 1 + images.length) % images.length); }}
                  style={{ position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", width: 30, height: 30, borderRadius: "50%", background: "rgba(255,255,255,0.85)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <FiChevronLeft size={16}/>
                </button>
                <button onClick={e => { e.stopPropagation(); setActiveImg(i => (i + 1) % images.length); }}
                  style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", width: 30, height: 30, borderRadius: "50%", background: "rgba(255,255,255,0.85)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ animation: "slideUp 0.4s ease" }}>
            {product.brand && (
              <p style={{ fontSize: 11, color: GOLD_DARK, fontWeight: 700, letterSpacing: 1, marginBottom: 4, textTransform: "uppercase" }}>
                {product.brand}
              </p>
            )}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 12, color: MUTED }}>{reviews.length || product.reviews || 0} Reviews</span>
            </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20, padding: "14px 16px", backgroundColor: GOLD_LIGHT, borderRadius: 8, border: `1px solid ${GOLD}44` }}>
              ...
              {discount > 0 && (
                <span style={{ fontSize: 12, fontWeight: 800, color: GREEN, backgroundColor: "#E8F5E9", padding: "2px 8px", borderRadius: 4 }}>
                  {discount}% OFF
                </span>
              )}
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: CHARCOAL, letterSpacing: 0.5 }}>
                    SIZE: {selSize && <span style={{ color: GOLD_DARK }}>{selSize}</span>}
                  </p>
              ...
              <div style={{ display: "flex", borderBottom: `2px solid ${BORDER}`, marginBottom: 16 }}>
                {["description","details","reviews"].map(t => (
                  <button key={t} onClick={() => setTab(t)}
                    ...
                    {t}{t === "reviews" && ` (${reviews.length || product.reviews || 0})`}
                  </button>
                ))}
              </div>
                ...
                <div style={{ animation: "fadeIn 0.3s ease" }}>
                  <div style={{ display: "flex", gap: 20, alignItems: "center", padding: 16, backgroundColor: GOLD_LIGHT, borderRadius: 8, marginBottom: 20, border: `1px solid ${GOLD}33` }}>
                    <div style={{ textAlign: "center", flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: MUTED, marginTop: 4 }}>{reviews.length || product.reviews || 0} reviews</div>
                    </div>
                    <div style={{ flex: 1 }}>
                      {ratingDist.map(r => (
                  ...
                  {reviews.length === 0
                    ...
                    : reviews.map((r, i) => (
                      <div key={i} style={{ padding: "14px 0", borderBottom: i < reviews.length - 1 ? `1px solid ${BORDER}` : "none" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                          <div style={{ width: 32, height: 32, borderRadius: "50%", background: `linear-gradient(135deg,${GOLD_LIGHT},${GOLD})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: GOLD_DARK, flexShrink: 0 }}>
                            {(r.name || "?").charAt(0)}
                          </div>
                          <div>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {relatedProds.length > 0 && (
          <div style={{ marginTop: 60, borderTop: `1px solid ${BORDER}`, paddingTop: 40 }}>
            <h2 style={{ fontSize: 20, fontWeight: 700, color: CHARCOAL, marginBottom: 8, fontFamily: "'Playfair Display',Georgia,serif", textAlign: "center" }}>
              YOU MIGHT ALSO LIKE
            </h2>
            {product.colTitle && (
              <p style={{ textAlign: "center", fontSize: 12, color: MUTED, marginBottom: 24 }}>
                More from {product.colTitle}
              </p>
            )}
      ...
      {viewerOpen && images.length > 0 && (
        <ImageViewer images={images} startIdx={activeImg} onClose={() => setViewerOpen(false)}/>
      )}
    </div>
  );
}