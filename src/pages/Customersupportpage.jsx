...
const SUPPORT_OPTIONS = [
  { icon: <FiMessageCircle size={24} />, title: 'Live Chat',   desc: 'Chat with us instantly',      badge: 'Online',    badgeColor: '#2E7D32', badgeBg: '#E8F5E9' },
  ...
  { icon: <FiSmartphone size={24} />,    title: 'WhatsApp',    desc: 'Message us on WhatsApp',      badge: 'Fast Reply', badgeColor: '#2E7D32', badgeBg: '#E8F5E9' },
];
      ...
      <div style={{ backgroundColor: SURFACE, minHeight: '100vh', fontFamily: "'DM Sans', sans-serif", animation: 'fadeUp 0.3s ease' }}>
        ...
        <div style={{ background: `linear-gradient(145deg,${CHARCOAL} 0%,#2C2416 60%,#3D2E10 100%)`, padding: '28px 20px 36px', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h1 style={{ fontSize: 22, fontWeight: 800, color: '#fff', fontFamily: "'Playfair Display', serif" }}>Customer Support</h1>
        ...
        <div style={{ padding: '20px 16px 100px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
            {SUPPORT_OPTIONS.map((opt, i) => (
              <div key={i} style={{ backgroundColor: '#fff', borderRadius: 14, border: `1px solid ${BORDER}`, padding: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', cursor: 'pointer' }}>
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: 14, border: `1px solid ${BORDER}`, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', marginBottom: 16 }}>
            ...
            <div style={{ padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {COMMON_ISSUES.map((issue, i) => (
                <button key={i} onClick={() => setSelected(issue.label)}
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: 14, border: `1px solid ${BORDER}`, overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            ...
            {!sent ? (
              <div style={{ padding: 16 }}>
                {selected && (
                  <div style={{ padding: '10px 14px', borderRadius: 8, backgroundColor: GOLD_LIGHT, border: `1px solid ${GOLD}`, marginBottom: 12 }}>
                ...
                <button onClick={() => message.trim() && setSent(true)}
                  style={{ marginTop: 12, width: '100%', padding: 14, background: `linear-gradient(to right,${GOLD_DARK},${GOLD})`, border: 'none', borderRadius: 12, color: '#fff', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit' }}>
                  Send Message
                </button>
              </div>
            ) : (
              <div style={{ padding: 40, textAlign: 'center' }}>
                <div style={{ marginBottom: 12 }}><FiSend size={48} color={GREEN} /></div>
                <h3 style={{ fontSize: 18, fontWeight: 800, color: CHARCOAL, fontFamily: "'Playfair Display', serif", marginBottom: 8 }}>Message Sent!</h3>
                <p style={{ fontSize: 13, color: MUTED }}>We'll reply to your registered email within 24 hours.</p>
              </div>
            )}
          </div>
        </div>
